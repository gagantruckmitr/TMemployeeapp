...
use App\Http\Controllers\API\PopupMessageApiController;
...
Route::middleware(['auth:api'])->post('/test-firebase', [AdminController::class, 'testNotification']);

// Route::middleware(['auth:api'])->get('/certificates', [CertificatesController::class, 'generateCertificates']);
...
Route::delete('/subscriptions/{id}', [SubscriptionController::class, 'destroy']);
  ...
  Route::middleware(['auth:api'])->get('/certificates/{moduleId?}', [CertificatesController::class, 'generateCertificate'])
        ->where('moduleId', '[0-9]+')
        ->name('certificates.generate');
    
    Route::middleware(['auth:api'])->get('/certificates/all', [CertificatesController::class, 'generateAllCertificates'])
...
Route::middleware('auth:api')->group(function () {
    ...
    Route::get('/payment/details', [PaymentController::class, 'details']);
    Route::post('/payment/capture', [PaymentController::class, 'capture']);
    Route::post('/payment/send-invoice', [PaymentController::class, 'sendInvoiceEmail']);
...
Route::get('/subscription-consent-and-disclaimer', [PublicController::class, 'subscriptionConsentAndDisclaimer']);
Route::get('/transporter-consent-for-job-posting-data-sharing', [PublicController::class, 'transporterConsentForJobPostingDataSharing']);
Route::get('/driver-consent-for-job-application-data-sharing', [PublicController::class, 'driverConsentForJobApplicationDataSharing']);
...
Route::middleware('auth:api')->post('/transporter/job-application/{applicationId}', [JobController::class, 'acceptRejectApplication']);
...
Route::middleware('auth:api')->group(function () {
    ...
    Route::post('/payment/send-invoice-email', [PaymentController::class, 'sendInvoiceEmail']);
    ...
    Route::get('/orders/payments_details', [PaymentController::class, 'getOrderPayments']);
});

// Banner API Routes (Protected)
Route::middleware('auth:api')->group(function () {
    Route::get('/banners', [App\Http\Controllers\API\BannerApiController::class, 'index']);
    Route::get('/banners/{id}', [App\Http\Controllers\API\BannerApiController::class, 'show']);
    Route::get('/banners/type/{type}', [App\Http\Controllers\API\BannerApiController::class, 'getByType']);
});
// Callback Request API Routes (Protected - for mobile apps)
Route::middleware('auth:api')->post('/callback-request', [App\Http\Controllers\CallbackRequestController::class, 'apiStore']);

// Popup Message API Routes (Protected)
Route::middleware('auth:api')->get('/popup-messages', [PopupMessageApiController::class, 'index']);

// Call Logs API Routes
Route::middleware('auth:api')->group(function () {
    Route::post('/call-logs/logCallTransporter', [CallLogApiController::class, 'logCallTransporter']);
    Route::put('/call-logs/{id}/status', [CallLogApiController::class, 'updateCallStatus']);
    Route::get('/call-logs/statistics', [CallLogApiController::class, 'getStatistics']);
});
