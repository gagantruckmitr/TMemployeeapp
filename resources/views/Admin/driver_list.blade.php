...
<form action="{{ route('driver.export') }}" method="GET" style="display:inline-block;">
  <input type="hidden" name="added_by" value="{{ request()->input('added_by') }}">
  <input type="hidden" name="state_name" value="{{ request()->input('state_name') }}">
  ...
  <button class="btn btn-success btn-sm" type="submit">Export to Excel <i class="fas fa-file-export"></i></button>
</form>
...
<button style="background-color: #1a6dba !important; border:0 !important; outline:0 !important" type="button" class="btn btn-primary btn-sm" data-bs-toggle="modal" data-bs-target="#filterModal">
   ...
   @if(request()->filled('added_by') || request()->filled('state_name') || request()->filled('from_date') || request()->filled('to_date') || request()->filled('status') || request()->filled('rating') || request()->filled('tier') || request()->filled('payment_status')) 
   <a href="{{ url('admin/driver-list') }}" class="btn btn-outline-secondary btn-sm">
      Clear Filter
   </a>
   @endif
		   ...
		   <form method="GET" action="{{ url('admin/driver-list') }}">
...
?>
                            <tr>
                                <td><a href="/admin/driver-applied-job/{{$list->id}}">{{$list->unique_id}}</a></td>
...
@endif
                                ...
                                <td>{{date('d-m-Y',strtotime($list->Created_at))}}</td>
                                 <td>
                                @if ($list->status == '1')
                                   <a href="{{url('admin/status_driver',$list->id)}}" onclick="return confirm('Are you sure you want to Inactive this Record?');"class="badge badge-success">Active</a>
                                @else
                                    <a href="{{url('admin/status_driver',$list->id)}}" onclick="return confirm('Are you sure you want to Active this Record?');"class="badge badge-warning">Inactive</a>
                                @endif
                                </td>

                                <td class="text-end">
                                    <div class=" ">
                                        <a class="edit-btn" href="{{url('admin/update-truck-driver')}}/{{$list->id}}">
                                            Edit
                                        </a>&nbsp;&nbsp;&nbsp;&nbsp;
                                        <a class="delete-btn" href="{{url('/admin/delete-truck-driver')}}/{{$list->id}}" onclick="return confirm('Are you sure to delete the record?')" class="btn btn-sm bg-danger-light">
                                            Delete
                                        </a>
                                    </div>
                                </td>
...
<form action="{{ url('admin/driver-list') }}" method="GET" id="filterForm">
  <div class="row">
    <!-- Driver Added By -->
    <div class="mb-3 col-md-6">
      ...
      <select class="form-select" name="added_by" id="addedBy">
        ...
        <option value="transporter" {{ ($filter_added_by ?? '') == 'transporter' ? 'selected' : '' }}>Transporter</option>
        <option value="self" {{ ($filter_added_by ?? '') == 'self' ? 'selected' : '' }}>Self</option>
      </select>
    </div>
    ...
    <div class="mb-3 col-md-6">
      ...
      <select class="form-select" id="stateName" name="state_name">
        ...
        @foreach ($states as $state)
          <option value="{{ $state->id }}" {{ (isset($filter_state) && $filter_state == $state->id) ? 'selected' : '' }}>
            {{ $state->name }}
          </option>
        @endforeach
      </select>
...
<div class="mb-3 col-md-6">
  ...
  <select class="form-select" id="payment_status" name="payment_status">
    ...
    <option value="captured" {{ request('payment_status') == 'captured' ? 'selected' : '' }}>Received</option>
    <option value="not_received" {{ request('payment_status') == 'not_received' ? 'selected' : '' }}>Not Received</option>
  </select>
</div>
</form>
      </div>
      ...
      <div class="modal-footer">
         <a href="{{ url('admin/driver-list') }}" class="btn btn-secondary btn-sm">
            Clear Filters
        </a>
