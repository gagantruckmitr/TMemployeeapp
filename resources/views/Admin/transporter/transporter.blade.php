...
<div class="page-wrapper">
    ...
    @if(request()->filled('state') || request()->filled('status') || request()->filled('from_date') || request()->filled('to_date'))
    ...
    {{-- Filter Button --}}
    <button style="background-color: #1a6dba !important; border:0 !important; outline:0 !important" type="button" class="btn btn-primary btn-sm" data-bs-toggle="modal" data-bs-target="#transporterFilterModal">
      <i class="fas fa-filter me-1"></i> Filter
    </button>
    <form action="{{ route('transporter.export') }}" method="GET" style="display:inline-block;">
                    ...
                    <div class="table-responsive">
                        <table class="table border-0 star-student table-hover table-center mb-0 datatable table-striped" id="dfUsageTable">
                            ...
                            <tr>
                                ...
                                <!--<td>{{ $i++ }}</td>-->
                                <td><a style="color:#3d5ee1;" href="{{ url('admin/transporter-job')}}/{{ $list->id }}">{{ $list->unique_id }}</a></td>
...
<a href="#" class="avatar avatar-sm me-2">
  <img class="avatar-img rounded-circle" src="{{ $imagePath }}" alt="User Image">
</a>
                                    </h2>
                                </td>
                                <td>{{$list->name}}</td>
    ...
    @endif
                               ...
                               <td>{{ \Carbon\Carbon::parse($list->Created_at)->setTimezone('Asia/Kolkata')->format('d-m-Y') }}</td>
                                <td>
                                @if ($list->status == '1')
                                   <a href="{{url('admin/status_transporter',$list->id)}}" onclick="return confirm('Are you sure you want to Inactive this Record?');"class="badge badge-success">Active</a>
                                @else
                                    <a href="{{url('admin/status_transporter',$list->id)}}" onclick="return confirm('Are you sure you want to Active this Record?');"class="badge badge-warning">Inactive</a>
                                @endif
                                </td>
                                ...
                                <td class="text-end">
                                    <div class=" ">
                                        ...
                                        <a class="delete-btn" href="{{url('/admin/delete_transporter')}}/{{$list->id}}" onclick="return confirm('Are you sure to delete the record?')" class="btn btn-sm bg-danger-light">
                                        Delete
                                        </a>
                                    </div>
                                </td>
...
<div class="modal fade" id="transporterFilterModal" tabindex="-1" aria-labelledby="transporterFilterModalLabel" aria-hidden="true">
...
@endforeach
  ...
  <select class="form-select" id="payment_status" name="payment_status">
    ...
    <option value="captured" {{ request('payment_status') == 'captured' ? 'selected' : '' }}>Yes</option>
    <option value="not_received" {{ request('payment_status') == 'not_received' ? 'selected' : '' }}>No</option>
  </select>
            </div>
          </div>
        </div>
        ...
        <div class="modal-footer d-flex">
          ...
          <button style="background-color: #1a6dba !important; border:0 !important; outline:0 !important" type="submit" class="btn btn-primary btn-sm">
            Apply Filter
          </button>
        </div>
      </form>
