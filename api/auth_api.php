...
function getUserStats($pdo, $userId, $role) {
    ...
    try {
        ...
        if ($stmt->rowCount() > 0) {
            ...
            $stmt = $pdo->prepare("SELECT COUNT(*) as count FROM call_logs WHERE caller_id = ?");
            $stmt->execute([$userId]);
            $stats['totalCalls'] = (int)$stmt->fetch()['count'];
            
            // Connected calls
            $stmt = $pdo->prepare("SELECT COUNT(*) as count FROM call_logs WHERE caller_id = ? AND call_status = 'connected'");
            $stmt->execute([$userId]);
            $stats['connectedCalls'] = (int)$stmt->fetch()['count'];
            
            // Pending calls
            $stmt = $pdo->prepare("SELECT COUNT(*) as count FROM call_logs WHERE caller_id = ? AND call_status = 'pending'");
            $stmt->execute([$userId]);
            $stats['pendingCalls'] = (int)$stmt->fetch()['count'];
            
            // Callbacks scheduled
            $stmt = $pdo->prepare("SELECT COUNT(*) as count FROM call_logs WHERE caller_id = ? AND (call_status = 'callback' OR call_status = 'callback_later')");
            $stmt->execute([$userId]);
