...
class ApiService {
  ...
  // Update call feedback after completion (Production Ready)
  static Future<bool> updateCallFeedback({
    ...
    try {
      // Use production TeleCMI API
      final telecmiUri = Uri.parse(
        '$baseUrl/telecmi_production_api.php',
      ).replace(queryParameters: {'action': 'update_feedback'});

      print('🔵 Updating TeleCMI call feedback:');
      ...
      print('🔵 Feedback Update Response: ${response.statusCode}');
      ...
      if (response.statusCode == 200) {
        ...
        if (data['success'] == true) {
          ...
          print('❌ Feedback update failed: ${data['message']}');
        }
      }

      // Fallback to IVR API for backward compatibility (manual calls)
      final ivrUri = Uri.parse(
        ApiConfig.ivrCallApi,
      ).replace(queryParameters: {'action': 'update_feedback'});

      final ivrResponse = await http
          .post(
            ivrUri,
            headers: {'Content-Type': 'application/json'},
            body: json.encode({
      ...
      if (ivrResponse.statusCode == 200) {
        final data = json.decode(ivrResponse.body);
        return data['success'] == true;
      }
  ...
  // Initiate TeleCMI IVR call (Production Ready)
  static Future<Map<String, dynamic>> initiateTeleCMICall({
    required String driverMobile,
    required int callerId,
    required String driverId,
  }) async {
    print('═══════════════════════════════════════════════════════');
    print('🌐 API SERVICE - initiateTeleCMICall()');
    print('═══════════════════════════════════════════════════════');
    ...
    try {
      // Use TeleCMI Flutter API (based on working telecmi_test_call.php)
      final uri = Uri.parse('$baseUrl/telecmi_flutter_api.php');
      ...
      if (response.statusCode == 200) {
        print('✅ HTTP 200 OK - Parsing JSON response...');
        ...
        try {
          ...
          if (data['data'] != null) {
            ...
            print('   Call ID: ${data['data']['call_id']}');
            print('   Request ID: ${data['data']['request_id']}');
          }
          
          if (data['success'] == true) {
            print('─────────────────────────────────────────────────────');
            print('✅ TeleCMI CALL INITIATED SUCCESSFULLY');
            print('═══════════════════════════════════════════════════════');
            return data;
          } else {
            print('─────────────────────────────────────────────────────');
            print('⚠️ TeleCMI API returned success=false');
          ...
          print('   Stack: $jsonStack');
          print('═══════════════════════════════════════════════════════');
          return {
            'success': false,
            'error': 'Failed to parse response: $jsonError',
          };
        }
      } else if (response.statusCode == 403) {
        print('─────────────────────────────────────────────────────');
        print('❌ HTTP 403 FORBIDDEN - Unauthorized');
        ...
        try {
          ...
          return {
            'success': false,
            'error': data['message'] ?? 'You are not authorized to use TeleCMI calling',
          };
        } catch (e) {
      ...
      print('═══════════════════════════════════════════════════════');
      print('❌ EXCEPTION in ApiService.initiateTeleCMICall()');
      print('═══════════════════════════════════════════════════════');
