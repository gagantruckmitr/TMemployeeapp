...
public function viewJob($id)
{
    try {
        ...
        // Format Application_Deadline only
...
public function allJobsForTransporter(Request $request)
{
    try {
        ...
        // Force format and override timestamps
        $jobs->transform(function ($job) {
            $job->Application_Deadline = $job->Application_Deadline
                ? Carbon::parse($job->Application_Deadline)->format('d-m-Y')
                : null;

            // Custom formatted fields
            $job->Created_at = Carbon::parse($job->created_at)->format('d-m-Y');
            $job->Updated_at = Carbon::parse($job->updated_at)->format('d-m-Y');

            // Hide default fields
            $job->makeHidden(['created_at', 'updated_at']);
...
public function recommendedJobs(Request $request)
{
    try {
        ...
        // Make sure the column names match DB structure
        $vehicleType = $user->vehicle_type;
        $drivingExp = (int) $user->Driving_Experience;
        ...
        $userLicense = $user->Type_of_License;
        ...
        $jobs = DB::table('jobs')
            ...
            ->get()
            ->filter(function ($job) use ($drivingExp, $currentIncome, $expectedIncome, $userLicense) {
                // Experience check
                if (!empty($job->Required_Experience)) {
                    $expStr = str_replace(' ', '', $job->Required_Experience);
                    if (strpos($expStr, '-') !== false) {
                        [$minExp, $maxExp] = explode('-', $expStr);
                    } else {
                        $minExp = $maxExp = (int) $expStr;
                    }

                    if ($drivingExp < (int) $minExp || $drivingExp > (int) $maxExp) {
                        return false;
                    }
                }
                ...
                if (!empty($job->Salary_Range)) {
                    $salaryStr = str_replace(' ', '', $job->Salary_Range);
                    if (strpos($salaryStr, '-') !== false) {
                        [$minSalary, $maxSalary] = explode('-', $salaryStr);
                    } else {
                        $minSalary = $maxSalary = (int) $salaryStr;
                    }

                    if ($currentIncome > (int) $maxSalary || $expectedIncome < (int) $minSalary) {
                        return false;
                    }
                }

                // License check
                if (!empty($job->Type_of_License) && $job->Type_of_License !== $userLicense) {
                    return false;
                }
