...
class _ModernJobCardState extends State<ModernJobCard> {
  ...
  Future<void> _checkAssignment() async {
    ...
    if (user != null && mounted) {
      setState(() {
        // Use the isAssignedToMe flag from the API (already calculated server-side)
        _isAssignedToMe = widget.job.isAssignedToMe;
        
        final jobAssignedTo = widget.job.assignedTo;
        final currentUserId = user.id;
        
        // ALWAYS log for debugging
        print('=== ASSIGNMENT CHECK (${widget.isSearchResult ? "SEARCH" : "NORMAL"}) ===');
        print('Job ID: ${widget.job.jobId}');
        print('Job Title: ${widget.job.jobTitle}');
        print('Job assignedTo: $jobAssignedTo (type: ${jobAssignedTo.runtimeType})');
        print('Job assignedToName: "${widget.job.assignedToName}"');
        print('Current User ID: $currentUserId (type: ${currentUserId.runtimeType})');
        print('Current User Name: ${user.name}');
        print('Comparison Result: $jobAssignedTo == $currentUserId = ${jobAssignedTo == currentUserId}');
        print('_isAssignedToMe: $_isAssignedToMe');
        print('Will Display: ${_isAssignedToMe ? "Assigned to You" : "Assigned to ${widget.job.assignedToName ?? "Unknown"}"}');
        print('==============================================');
      });
    }
  }
  ...
  String _getSubscriptionDate() {
    ...
    try {
      ...
      final month = months[createdDate.month - 1];
          ...
          Expanded(
            child: Column(
              ...
              children: [
                ...
                Row(
                  children: [
                    Icon(Icons.calendar_today, size: 11, color: Colors.green.shade600),
                    const SizedBox(width: 3),
                    Flexible(
                      child: Text(
                        'Subscribed: ${_getSubscriptionDate()}',
                        style: TextStyle(
                          fontSize: 10,
              ...
              Expanded(
                child: Column(
                  ...
                  children: [
                    ...
                    // Show assignment badge ONLY when job is assigned to someone else
                    if (!_isAssignedToMe && widget.job.assignedTo != null) ...[
                      const SizedBox(height: 4),
                      Container(
                        ...
                        child: Text(
                          widget.job.assignedToName != null && widget.job.assignedToName!.isNotEmpty
                              ? 'Assigned to ${widget.job.assignedToName}'
                              : 'Assigned to Another Telecaller',
                          style: TextStyle(
                            fontSize: 10,
                            fontWeight: FontWeight.w600,
      ...
      children: [
        Expanded(
          child: OutlinedButton.icon(
            onPressed: () {
              // Block access if job is not assigned to current user
              if (!_isAssignedToMe) {
                ScaffoldMessenger.of(context).showSnackBar(
                  SnackBar(
                    content: Text(
                      'This job is assigned to ${widget.job.assignedToName ?? "another telecaller"}. You cannot view applicants.',
                    ),
                    backgroundColor: Colors.orange,
              ...
              if (widget.job.applicantsCount > 0) {
                // Validate jobId before navigation
                if (widget.job.jobId.isEmpty) {
                  ScaffoldMessenger.of(context).showSnackBar(
                    const SnackBar(
                      content: Text('Invalid job ID. Cannot view applicants.'),
            ...
            icon: Icon(
              ...
              size: 16,
              color: !_isAssignedToMe ? Colors.grey : null,
            ),
            label: Text(
            ...
            style: OutlinedButton.styleFrom(
              foregroundColor: _isAssignedToMe ? AppColors.primary : Colors.grey,
              side: BorderSide(
                color: _isAssignedToMe ? Colors.grey.shade300 : Colors.grey.shade400,
              ),
        ...
        const SizedBox(width: 8),
        Material(
          color: (widget.isSearchResult || !_isAssignedToMe) 
              ? Colors.grey.shade400 
              : Colors.green,
          borderRadius: BorderRadius.circular(10),
          child: InkWell(
            onTap: (widget.isSearchResult || !_isAssignedToMe)
                ? () {
                    // Disabled in search results or for non-assigned jobs
                    ScaffoldMessenger.of(context).showSnackBar(
                      SnackBar(
                        content: Text(
                          widget.isSearchResult
                              ? 'Call feature is disabled in search results'
